/* global UndoFlag.NoSwapFile, Dialog, StdButton.No, StdIcon.Question, StdButton.Cancel, StdButton.Yes */
...
function GraphDimensions(xMin, yMin, xMax, yMax, hasAxis){
     ...
     */
    this.getYMin = function (){ return yMin; };
 ...
 * @returns {Graph}
 */
function Graph(graphDimensions, xLabel, yLabel, bitmap, xZoom, yZoom) {
    this.axisColor = 0xFF888888;
    
    // module data
    let xMin_ = graphDimensions.getXMin();
    let yMin_ = graphDimensions.getYMin();
    let xMax_ = graphDimensions.getXMax(xZoom);
    let yMax_ = graphDimensions.getYMax(yZoom);
